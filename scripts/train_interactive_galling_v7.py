"""
Training Script for Interactive Galling Model V7 - Extended Training Data

V7 extends V6 by adding 25°C (room temperature) data as additional baseline
training. The 25°C data is mapped to T=160°C (ΔT=0) since both represent
"no galling" conditions — providing ~331 extra baseline transitions.

Architecture identical to V6. Only change is training data.

Usage:
    python scripts/train_interactive_galling_v7.py                    # Train from scratch
    python scripts/train_interactive_galling_v7.py --resume-from-best # Continue from best
    python scripts/train_interactive_galling_v7.py --validate-only    # Only validate
...
from src.models.interactive_galling_model_v7 import InteractiveGallingModelV7
...
def train_model(model, T_all, mu_all, delta_all, config, temp_list, device='cpu'):
    """
    Train V7 via direct MLE on observed transitions.
...
def main():
    parser = argparse.ArgumentParser(description='Train Interactive Galling Model V7')
    ...
    print("=" * 80)
    print("TRAINING INTERACTIVE GALLING MODEL V7 - EXTENDED DATA")
    print("=" * 80)
    print("\nV7 Key Change:")
    print("  Same architecture as V6 (Gaussian mixture transitions)")
    print("  ADDED: 25°C data mapped to T=160°C (ΔT=0) as baseline")
    print("  Training data: 25°C→160°C, 165°C, 167.5°C, 170°C")
    print("  ~331 extra baseline transitions from 25°C data")
    print("=" * 80)
    ...
    config = {
        ...
        'n_epochs': 5000,
        'patience_limit': 500,
        'save_dir': 'results/interactive_galling_v7',
    }
    ...
    # ======================================================================
    print("\n" + "-" * 80)
    print("Creating V7 model...")
    print("-" * 80)

    model = InteractiveGallingModelV7(device=device)
    ...
    # ======================================================================
    print("\n" + "-" * 80)
    print("Training V7 model (direct MLE)...")
    print("-" * 80)
    ...
    # ======================================================================
    print("\n" + "-" * 80)
    print("Validating trained V7 model...")
    print("-" * 80)
    ...
    ax.set_title('V7 Training Progress')
    ax.grid(alpha=0.3)
    ...
    fig.suptitle('V7: Simulated vs Observed COF', fontsize=14)
    ...
    # Plot 3: Learned transition landscape (include 160°C baseline)
    fig, axes = plt.subplots(2, 4, figsize=(20, 8))
    fig.suptitle('V7: Learned Transition Parameters', fontsize=14)

    mu_range = torch.linspace(0.1, 1.3, 100, device=device)

    for col_idx, temp in enumerate([160, 165, 167.5, 170]):
    ...
    # ======================================================================
    print("\n" + "=" * 80)
    print("TRAINING SUMMARY - V7 EXTENDED DATA")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("V7 KEY CHANGES FROM V6:")
    print("=" * 80)
    print("  - ADDED 25°C (room temp) data mapped to 160°C (baseline)")
    print("  - ~331 extra baseline transitions for better low-state learning")
    print("  - Same architecture: 2-component Gaussian mixture")
    print("  - Same 17 learnable analytic parameters")
    print("  - Training data: 25°C→160°C, 165°C, 167.5°C, 170°C")
    print("=" * 80)
