"""
Interactive Galling Model V3 - Probabilistic Competition Dynamics
...
class InteractiveGallingModelV3(nn.Module):
    """
    Physics-Informed Interactive Galling Model V3 - Probabilistic Competition
    ...
    def __init__(
        ...
        if physics_init is None:
            physics_init = {
                ...
                # --- Output Noise (measurement only, small) ---
                'output_noise': 0.02,  # Small measurement uncertainty
            }
    ...
    def compute_friction(self, rho: torch.Tensor):
        ...
        V3: Very small output noise (measurement uncertainty only).
        Main variability comes from probabilistic competition, not noise.
        """
        logit = self.physics_params['beta_sharpness'] * (rho - self.physics_params['rho_crit'])
        beta = torch.sigmoid(logit)
    ...
    def simulate_multiple_cycles(
        ...
        """Simulate multiple cycles with V3 probabilistic competition."""
        ...
        return {
            ...
            'p_growth_history': torch.cat(p_growth_list),      # NEW: probability tracking
            'outcome_history': torch.cat(outcome_list)          # NEW: outcome tracking
        }
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Interactive Galling Model V3 - Probabilistic Competition")
    print("=" * 70)
    ...
    fig.suptitle('Interactive Galling Model V3 - Probabilistic Competition', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig('v3_model_test.png', dpi=150)
    print("\nSaved: v3_model_test.png")
    plt.show()
