...
class PINNTrainer:
    ...
    def stage1_pretrain_nn(
        ...
        train_dataset, val_dataset = random_split(
            ...
            [train_size, val_size],
            generator=torch.Generator().manual_seed(self.config['validation']['random_seed'])
        )
    ...
    def stage2_joint_training(
        ...
        # Group data by temperature for multi-cycle simulation
        temp_groups = self._group_by_temperature(dataset)
        ...
        for epoch in range(n_epochs):
            ...
            # Train on each temperature group
            ...
            if (epoch + 1) % self.config['output']['save_every_n_epochs'] == 0:
                self._save_checkpoint(epoch, total_loss, stage=2)
            ...
            if total_loss < best_total_loss - min_delta:
                ...
                if self.config['output']['save_best_only']:
                    self._save_checkpoint(epoch, total_loss, stage=2, best=True)
                    if verbose:
                        print(f"  → Saved best model (loss: {total_loss:.6f})")
            else:
                patience_counter += 1
